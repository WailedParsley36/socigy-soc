...
namespace Socigy.Middlewares
{
    public class InternalOnlyMiddleware : IMiddleware
    {
        ...
        private readonly InternalCredVerifier.InternalCredVerifierClient _Verifier;
        public InternalOnlyMiddleware(ILogger<InternalOnlyMiddleware> logger, InternalCredVerifier.InternalCredVerifierClient verifier)
        {
            _Logger = logger;
            _Verifier = verifier;
        }
        ...
        public async Task<bool> IsInternalRequest(HttpContext context)
        {
            ...
            if (string.IsNullOrEmpty(clientIdHeader.Value.ToString()) || string.IsNullOrEmpty(clientSecretHeader.Value.ToString()))
                return false;

            _Logger.LogInformation("Verifying internal request client details");

            return (await _Verifier.VerifyAsync(new ClientDetails()
            {
