...
const compiled = await WebAssembly.compile(fs.readFileSync("../example-plugin/pkg/example_plugin_bg.wasm"));
let output = null;
function colorizeLogs(message) {
    let index = message.indexOf("INFO");
    if (index !== -1) {
        return (message.slice(0, index) + "\x1b[36mINFO\x1b[0m" + message.slice(index + 4));
    }
    ...
    if (index !== -1) {
        return (message.slice(0, index) + "\x1b[33mWARN\x1b[0m" + message.slice(index + 4));
    }
...
globalThis.socigy = {
    logging: {
        log: (message) => console.log(colorizeLogs(message)),
        error: (message, showUi, uiDelay) => {
            console.error(colorizeLogs(message));
        },
        fatal: (message, uiDelay) => {
            console.error(`\x1b[31mAPI-FATAL:\x1b[0m Plugin has exited with: ${message}`);
            output = null;
        },
    },
    permissions: {
        getDeclaredPermissions: (callbackId) => {
            TestPlugin.invoke_rust_callback(callbackId, JSON.stringify(permissions.getDeclaredPermissions()));
        },
        getPermissionsAsync: (callbackId) => {
            TestPlugin.invoke_rust_callback(callbackId, JSON.stringify(permissions.getPermissions()));
        },
        requestPermissionsAsync: (callbackId, requestedPermissions) => {
            apiLog("Plugin requested these permissions: ", requestedPermissions);
            TestPlugin.invoke_rust_callback(callbackId, JSON.stringify(permissions.requestPermissions(requestedPermissions)));
        },
    },
    device: {},
    utils: {
    ...
    ui: {
        ...
        render: {
            processComponentRenderChanges(id, changes_str) {
                let changes = JSON.parse(changes_str);
                apiLog("Renderer sent changes for component", id, changes);
                onChange(changes);
            },
            processAppRenderChanges(id, changes_str) { },
        },
        ...
        onAppWindowRender: (id, instance, result) => {
...
async function v3() {
    let vdom;
    function traverse(vdom, path, index = 1) {
    ...
    onChange = (changes) => {
        ...
        changes.forEach((x) => {
            ...
            switch (x.type) {
                case "updateChildren":
                    result.children[x.path[x.path.length - 1]].children = x.children;
                    break;
                case "updateChild":
                    result.children[x.path[x.path.length - 1]] = x.child;
                    break;
            }
        });
        apiLog("Updated VDOM", vdom);
    };
    const rawRender = TestPlugin.render_component(components.values().next().value, '{"renderString": true,"content":"Nazdar","imageUrl":"https://socigy.com/favicon/favicon.svg"}');
    apiLog("Plugin component render:", rawRender);
    vdom = JSON.parse(rawRender);
    apiLog("Plugin VDOM:", vdom);
    TestPlugin.invoke_ui_event(vdom.events.onLayout[0], JSON.stringify({
        type: "onLayout",
        layout: { x: 0, y: 0, width: 0, height: 0 },
    }));
    TestPlugin.render_component(components.values().next().value, '{"renderString": false,"content":"Nazdar","imageUrl":"https://socigy.com/favicon/favicon.svg"}');
